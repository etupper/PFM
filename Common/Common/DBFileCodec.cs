...
namespace Common {
	 ...
	 */
    public class PackedFileDbCodec : DbCodec {
		...
		public string TypeName {
			...
			set {
				if (!DBTypeMap.Instance.IsSupported (value)) {
					throw new DBFileNotSupportedException (string.Format ("No DB definition found for {0}", type));
				}
				type = DBTypeMap.Instance [value, DBTypeMap.Instance.MaxVersion (value)];
			}
		}
		#endregion
		
		#region Constructors
		public PackedFileDbCodec (PackedFile packedFile) : this(DBFile.typename (packedFile.Filepath)) {
		}
		public PackedFileDbCodec (string type) {
			TypeName = type;
		}
		public PackedFileDbCodec (TypeInfo info) : this(info.name) {
			Type = info;
		}
		#endregion
		...
		public DBFile readDbFile(PackedFile file) {
			return readDbFile (file.Data);
		}
		public DBFile readDbFile(byte[] data) {
			return readDbFile (new MemoryStream (data, 0, data.Length));
		}
		/*
		 ...
		 * Can be used to forced a different version than what's in the stream.
		 * Note that the header that was read will contain the version from
		 * the file, it probably needs to be corrected by callers.
		 */
        public DBFile readDbFile(Stream stream, TypeInfo realInfo) {
        ...
        public static DBFileHeader readHeader(PackedFile file) {
            using (MemoryStream stream = new MemoryStream(file.Data, 0, (int) file.Size)) {
